...
class APIProcessor:
    ...
    def get_answer_from_rag_context(self, question, rag_context, schema, model):
        ...
        # 规范化返回结构：保留已有 final_answer，避免无字段时静默变成 N/A
        if not isinstance(answer_dict, dict):
            raise RuntimeError(f"LLM 返回非 dict 类型: {type(answer_dict)}")

        # API 错误体被误解析时直接抛错
        if "status_code" in answer_dict and "final_answer" not in answer_dict:
            raise RuntimeError(f"DashScope 返回异常: {answer_dict}")

        if "final_answer" not in answer_dict:
            # 尝试从原始文本字段里再解析一层
            raw = answer_dict.get("content") or answer_dict.get("text") or ""
            if isinstance(raw, str) and raw.strip():
                if hasattr(self.processor, "_parse_content_to_dict"):
                    answer_dict = self.processor._parse_content_to_dict(raw)

        if "final_answer" not in answer_dict:
            raise RuntimeError(f"LLM 返回缺少 final_answer 字段: {answer_dict}")

        answer_dict.setdefault("step_by_step_analysis", "")
        answer_dict.setdefault("reasoning_summary", "")
        answer_dict.setdefault("relevant_pages", [])
        return answer_dict
...
class BaseDashscopeProcessor:
    ...
    def send_message(
        ...
        **kwargs
        ...
        # 先检查 API 调用是否成功，避免把错误 JSON 误当成答案
        status_code = getattr(response, 'status_code', None)
        if status_code is not None and status_code != 200:
            err_code = getattr(response, 'code', '') or ''
            err_msg = getattr(response, 'message', '') or str(response)
            raise RuntimeError(
                f"DashScope API 调用失败 (status={status_code}, code={err_code}): {err_msg}"
            )
        ...
        if hasattr(response, 'output') and response.output is not None and hasattr(response.output, 'choices'):
            ...
            raise RuntimeError(f"DashScope 返回异常，无有效 output: {response}")
        ...
        # 解析模型输出为结构化 dict（兼容 markdown 代码块与松散 JSON）
        parsed_content = self._parse_content_to_dict(content)
        return parsed_content

    def _parse_content_to_dict(self, content: str) -> dict:
        """将 Qwen 返回文本解析为答案 dict，避免解析失败后被上层兜底成 N/A。"""
        ...
        candidates = []
        # 1) 去掉 ``` / ```json 代码块包裹
        if content_str.startswith('```') and content_str.count('```') >= 2:
            first = content_str.find('```') + 3
            newline = content_str.find('\n', first)
            start = newline + 1 if newline > 0 else first
            end = content_str.rfind('```')
            if end > start:
                candidates.append(content_str[start:end].strip())
        candidates.append(content_str)

        # 2) 提取首尾花括号之间的 JSON 对象
        left = content_str.find('{')
        right = content_str.rfind('}')
        if left != -1 and right > left:
            candidates.append(content_str[left:right + 1])
