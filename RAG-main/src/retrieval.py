...
import faiss
from src.faiss_io import faiss_read_index
...
class VectorRetriever:
    ...
    def _load_dbs(self):
        ...
        for document_path in all_documents_paths:
            ...
            if not faiss_path.exists():
                ...
                continue
            try:
                vector_db = faiss_read_index(faiss_path)
            except Exception as e:
    ...
    def retrieve_by_company_name(self, company_name: str, query: str, llm_reranking_sample_size: int = None, top_n: int = 3, return_parent_pages: bool = False) -> List[Tuple[str, float]]:
        ...
        for target_report in target_reports:
            ...
            if not sha1:
                _log.warning(f"Skip document without sha1 for company '{company_name}'")
                continue
            faiss_path = self.vector_db_dir / f"{sha1}.faiss"
            if not faiss_path.exists():
                _log.warning(f"No vector DB found for sha1={sha1}, skip")
                continue
            ...
            if not chunks:
                continue
            # 每份文档取足够候选，最后再全局截断 top_n
            local_k = min(max(top_n, llm_reranking_sample_size or top_n), len(chunks))
            distances, indices = vector_db.search(x=embedding_array, k=local_k)
            for distance, index in zip(distances[0], indices[0]):
                ...
                all_candidates.append({
                    ...
                    "source_file": document.get("metainfo", {}).get("file_name", ""),
                })

        # IndexFlatIP：分数越高越相似
        all_candidates.sort(key=lambda x: x["distance"], reverse=True)
        ...
        for item in all_candidates:
            ...
            if return_parent_pages and parent_page:
                page_key = (source_file, parent_page["page"])
        ...
        for target_report in target_reports:
            ...
            if pages:
                for page in sorted(pages, key=lambda p: p["page"]):
                    all_pages.append({
                        "distance": 0.5,
                        "page": page_offset + page["page"],
                        "text": page["text"]
                    })
                if pages:
                    page_offset += max(p["page"] for p in pages) + 1
            else:
                # 纯文本/markdown 分块无 pages 时，回退到 chunks
