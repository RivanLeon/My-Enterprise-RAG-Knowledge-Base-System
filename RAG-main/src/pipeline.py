...
class PipelineConfig:
    def __init__(self, root_path: Path, subset_name: str = "subset.csv", questions_file_name: str = "questions.json", pdf_reports_dir_name: str = "pdf_reports", text_reports_dir_name: str = "text_reports", serialized: bool = False, config_suffix: str = ""):
        # 路径配置，支持不同流程和数据目录
        ...
        # 纯文本知识源目录（与 pdf_reports 并列）
        self.text_reports_dir = root_path / text_reports_dir_name
...
class Pipeline:
    def __init__(self, root_path: Path, subset_name: str = "subset.csv", questions_file_name: str = "questions.json", pdf_reports_dir_name: str = "pdf_reports", text_reports_dir_name: str = "text_reports", run_config: RunConfig = RunConfig()):
        # 初始化主流程，加载路径和配置
        self.run_config = run_config
        self.paths = self._initialize_paths(root_path, subset_name, questions_file_name, pdf_reports_dir_name, text_reports_dir_name)
        self._convert_json_to_csv_if_needed()

    def _initialize_paths(self, root_path: Path, subset_name: str, questions_file_name: str, pdf_reports_dir_name: str, text_reports_dir_name: str = "text_reports") -> PipelineConfig:
        """根据配置初始化所有路径"""
        return PipelineConfig(
            ...
            text_reports_dir_name=text_reports_dir_name,
    ...
    def chunk_reports(self, include_serialized_tables: bool = False):
        """
        将规整后 markdown 报告与纯文本报告分块，便于后续向量化和检索。
        保留原有 PDF→markdown 分块逻辑，并新增 text_reports 目录下的纯文本入库。
        """
        text_splitter = TextSplitter()
        # 1) 处理 markdown（通常由 PDF 解析得到）
        ...
        # 2) 处理纯文本知识源（.txt）
        text_dir = self.paths.text_reports_dir
        if text_dir.exists():
            print(f"开始分割 {text_dir} 目录下的纯文本文件...")
            text_splitter.split_text_reports(
                all_txt_dir=text_dir,
                output_dir=self.paths.documents_dir,
                subset_csv=self.paths.subset_path
            )
        else:
            print(f"未找到纯文本目录 {text_dir}，跳过文本分块")
        print(f"分割完成，结果已保存到 {self.paths.documents_dir}")

    def create_vector_dbs(self, skip_existing: bool = False):
        """从分块报告创建向量数据库；skip_existing=True 时跳过已有 faiss，避免重复嵌入"""
...
max_config = RunConfig(
    ...
    parallel_requests=4,
    submission_file=True,
    pipeline_details="Custom pdf parsing + vDB + Router + Parent Document Retrieval + reranking + SO CoT; llm = qwen-flash",
