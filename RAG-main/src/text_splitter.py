...
class TextSplitter():
    ...
    def _load_subset_maps(self, subset_csv: Optional[Path] = None):
        """从 subset.csv 建立文件名到 company_name / sha1 的映射"""
        file2company = {}
        file2sha1 = {}
        if subset_csv is None or not os.path.exists(subset_csv):
            return file2company, file2sha1
        try:
            df = pd.read_csv(subset_csv, encoding='utf-8')
        except UnicodeDecodeError:
            print('警告：subset.csv 不是 utf-8 编码，自动尝试 gbk 编码...')
    ...
    def split_text_file(self, txt_path: Path, chunk_size: int = 30, chunk_overlap: int = 5):
        """
        按行分割纯文本文件，分块格式与 markdown 一致，便于统一入库。
        :param txt_path: 纯文本文件路径
        :param chunk_size: 每个分块的最大行数
        :param chunk_overlap: 分块重叠行数
        :return: 分块列表
        """
        # 复用 markdown 按行分块逻辑（纯文本同样按行切分）
        return self.split_markdown_file(txt_path, chunk_size=chunk_size, chunk_overlap=chunk_overlap)

    def split_text_reports(
        self,
        all_txt_dir: Path,
        output_dir: Path,
        chunk_size: int = 30,
        ...
        """
        批量处理目录下纯文本文件（默认 .txt），分块并输出为 json，供向量化使用。
        保留与 markdown 分块相同的输出结构，便于检索侧统一处理。
        """
        if extensions is None:
            extensions = [".txt"]
        file2company, file2sha1 = self._load_subset_maps(subset_csv)

        all_txt_paths = []
        for ext in extensions:
            all_txt_paths.extend(all_txt_dir.glob(f"*{ext}"))
        # 去重并保持稳定顺序
        all_txt_paths = sorted(set(all_txt_paths), key=lambda p: p.name)

        output_dir.mkdir(parents=True, exist_ok=True)
        for txt_path in all_txt_paths:
            chunks = self.split_text_file(txt_path, chunk_size, chunk_overlap)
            output_json_path = output_dir / (txt_path.stem + ".json")
            file_no_ext = txt_path.stem
            company_name = file2company.get(file_no_ext, "")
            sha1 = file2sha1.get(file_no_ext, "")
            if not sha1:
                # 未在 subset 登记时，用文件内容生成稳定 sha1，保证 faiss 可落盘
                with open(txt_path, 'rb') as f:
                    sha1 = hashlib.sha1(f.read()).hexdigest()
                print(f"警告：{txt_path.name} 未在 subset.csv 登记，自动生成 sha1={sha1}")
            if not company_name:
                print(f"警告：{txt_path.name} 未匹配到 company_name，检索可能无法按公司命中")
            metainfo = {
                "sha1": sha1,
                "company_name": company_name,
            ...
            with open(output_json_path, 'w', encoding='utf-8') as f:
                json.dump({"metainfo": metainfo, "content": {"chunks": chunks}}, f, ensure_ascii=False, indent=2)
            print(f"已处理纯文本: {txt_path.name} -> {output_json_path.name}")
        print(f"共分割 {len(all_txt_paths)} 个纯文本文件")
