...
class AnswerWithRAGContextSharedPrompt:
    instruction = """
你是一个RAG（检索增强生成）问答系统。
你的任务是仅基于知识库检索到的相关内容回答给定问题。
检索内容可能来自公司年报、季度报告、公开披露整理稿或其他纯文本资料，只要上下文中有明确信息即可使用，不要仅因不是“年报原文”而拒绝作答。

在给出最终答案前，请详细分步思考，尤其关注问题措辞。
- 注意：答案可能与问题表述不同。
- 问题可能是模板生成的，有时对该公司不适用。
- 若上下文已明确给出营收、利润、产能利用率等指标，应直接引用作答。
"""
...
示例：
问题：
"请简要说明中芯国际2025年第一季度的营收和产能利用率。"

答案：
```
{
  "step_by_step_analysis": "1. 问题询问2025年第一季度营收与产能利用率两个指标。\n2. 检索到来源文件「中芯国际_2025年一季度业绩要点.txt」，其中明确披露相关数据。\n3. 营收（人民币口径）为163.01亿元，同比增长约29.4%。\n4. 产能利用率为89.6%，环比提升约4.1个百分点。\n5. 上述信息可直接用于回答，无需推断。",
  "reasoning_summary": "一季度要点文本已直接给出营收163.01亿元与产能利用率89.6%。",
  "relevant_pages": [0],
  "final_answer": "中芯国际2025年第一季度营业收入为163.01亿元人民币，产能利用率为89.6%。"
}
```
'''
